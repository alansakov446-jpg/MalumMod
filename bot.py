...
Нужные переменные окружения:
    ...
    GEMINI_API_KEY    — бесплатный ключ Google Gemini (aistudio.google.com)
...
# Список историй диалога по chat_id, чтобы Ириска помнила контекст переписки
...
async def ask_gemini(chat_id: int, question: str) -> str:
    ...
    if resp.status_code != 200:
        ...
        return (
            "Не смогла обратиться к ИИ (ошибка API). "
...
async def main() -> None:
    ...
    async def handle_message(message: Message) -> None:
        ...
        try:
            answer = await ask_gemini(message.chat.id, question)
        except Exception:
